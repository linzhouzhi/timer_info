...
//判断参数是否合法
if( empty($_SERVER["REMOTE_ADDR"]) || !isset($_SERVER["REMOTE_ADDR"]) || empty($_GET['url']) || !isset($_GET['exe_time']) || !isset($_GET['type']) || !isset($_GET['interval_time']) ){
	echo "您的请求参数不符合格式！";
}else{
	...
	if( !empty($check_ip) ){  //检查ip是否在白名单中
		...
		if( !empty($check_url) ){
			//数据库有相应的记录则更新
			$DService->update_pass_url( $exe_time,$type,$interval_time,$url );
		}else{
			//否则直接插入
			$DService->insert_respon_url( $exe_time,$type,$interval_time,$url );
		}
	}else{
		echo "您没有权限访问";
	}
